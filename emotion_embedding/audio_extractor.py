...
import torch
import torchaudio
# Import YOUR specific audio model here (e.g., AST, wav2vec, etc.)
...
def load_wav_file(file_path, target_sr=16000):
    ...
    # 2. Convert to Mono (if it's a stereo file with 2 channels)
    if waveform.shape[0] > 1:
        waveform = torch.mean(waveform, dim=0, keepdim=True)
        
    # 3. Resample to 16000 Hz (The standard for AST models)
    if sample_rate != target_sr:
        resampler = torchaudio.transforms.Resample(orig_freq=sample_rate, new_freq=target_sr)
        waveform = resampler(waveform)
        
    # 4. Add the Batch Dimension
    # torchaudio loads as [Channels, Time]. We need [Batch, Channels, Time] -> [1, 1, Time]
    # Depending on your specific AAST implementation, you might need to drop the channel dim: waveform.squeeze(0).unsqueeze(0)
...
def bake_emotions():
    ...
    # 1. Load your trained audio model
    ...
    # 2. Define the classic audio files you want to use as your "Core Emotions"
    emotion_files = {
        "Anger": "anger.wav"
    }
    ...
    with torch.no_grad():
        for emotion_name, file_path in emotion_files.items():
            try:
                ...
                # Pass it through the network to get the [1, 64] vector
                latent_code = audio_model(audio_tensor)
                
                # Squeeze it down to a flat 1D list of 64 numbers
                flat_list = latent_code.squeeze().tolist()
                
                # Format it nicely so you can easily copy-paste it into Unreal Engine
